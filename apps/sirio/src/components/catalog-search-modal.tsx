...
function SearchResultItem({
  ...
  return (
    <Link
      ...
      className="group/search-result relative z-10 flex items-center justify-between p-2.5 rounded-lg border border-border/40 bg-card/40 transition-colors duration-150 text-xs"
    >
      <div className="flex items-center gap-3">
        <div className="p-1.5 rounded-md bg-muted text-muted-foreground group-hover/search-result:bg-primary group-hover/search-result:text-primary-foreground group-focus-visible/search-result:bg-primary group-focus-visible/search-result:text-primary-foreground transition-colors">
          <item.icon aria-hidden="true" className="size-4" />
        </div>
        <span className="font-semibold font-accent text-foreground group-hover/search-result:text-primary transition-colors">
          {item.name}
        </span>
      </div>
      ...
      <DialogContent closeButtonProps={{ "aria-label": "Chiudi ricerca nel catalogo" }} className="p-0 gap-0 sm:max-w-lg overflow-hidden border-border/80 bg-popover/95 backdrop-blur-xl shadow-2xl">
        <DialogHeader className="p-4 border-b border-border/60 pb-3">
          <div className="flex items-center justify-between pr-8">
            ...
            <KbdShortcut value="⌘K" className="text-xs" />
          </div>
          <div className="mt-3">
            <SearchField
              aria-label="Cerca nel catalogo Sirio"
              ref={inputRef}
        ...
        <div className="max-h-[60vh] overflow-y-auto p-3 space-y-1">
            <SearchResults
              ...
              onReset={() => { setQuery(""); inputRef.current?.focus(); }}
            >
