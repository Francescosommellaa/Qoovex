...
- usa la sintassi canonica Tailwind v4 per i token: `gap-4` invece di `gap-[var(--spacing-4)]`, `text-(length:--text-sm)` invece di `text-[length:var(--text-sm)]`, `bg-(--token)` / `border-(--token)` / `text-(--token)` invece di `bg-[var(--token)]`.
