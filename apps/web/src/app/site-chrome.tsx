...
const mainLinks = [{ href: "/", label: "Prodotto" }, { href: "/contattaci", label: "Contattaci" }];
...
export function SiteFooter() { return <footer className="border-t bg-card"><div className="mx-auto flex max-w-7xl flex-col gap-8 px-4 py-12 sm:px-6"><div><BrandMark /><p className="mt-4 max-w-2xl text-sm leading-6 text-muted-foreground">Qoovex documenta il processo condiviso. Non movimenta denaro, non certifica conformità e non sostituisce il giudizio professionale.</p><a className="mt-3 inline-block text-sm text-muted-foreground hover:text-foreground" href={contactHref}>{contactEmail}</a></div><nav className="flex flex-wrap gap-4 text-sm" aria-label="Link legali">{legalLinks.map((link) => <a href={link.href} key={link.href}>{link.label}</a>)}</nav><p className="text-xs text-muted-foreground">© 2026 Qoovex.</p></div></footer>; }
