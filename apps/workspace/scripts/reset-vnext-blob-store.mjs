...
const RESET_ATTESTATION = "RESET_QOOVEX_VNEXT_EMPTY_BLOB_STORE_2026_08_03";
...
if (environment !== "preview" && environment !== "production") {
  throw new Error("vNext Blob reset is allowed only in Preview or Production.");
}
if (databaseEnvironment !== environment) {
  throw new Error("Blob reset refused: database and Vercel environment markers differ.");
}
if (process.env.QOOVEX_VNEXT_BLOB_RESET_APPROVED !== RESET_ATTESTATION) {
  throw new Error("Exact vNext Blob reset attestation is required.");
}
...
console.log(`vNext Blob reset completed: ${pathnames.length} object(s) removed.`);
