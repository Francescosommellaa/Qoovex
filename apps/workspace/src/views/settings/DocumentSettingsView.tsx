...
export function DocumentSettingsView({
  ...
  return (
    <WorkspacePage>
      ...
      <section aria-labelledby="unclassified" className="grid gap-3" id="unclassified"><div><h2 className="text-lg font-medium">Da classificare</h2><p className="mt-1 text-sm text-muted-foreground">Dati legacy mantenuti visibili senza attribuire automaticamente una categoria.</p></div><Card size="sm"><CardHeader><CardTitle><h3>Tipi senza categoria</h3></CardTitle><CardDescription>{unclassifiedTypes.length} tipi richiedono una scelta OWNER o ADMIN.</CardDescription></CardHeader><CardContent className="grid gap-3">{!unclassifiedTypes.length ? <p className="flex items-center gap-2 text-sm text-muted-foreground"><IconCircleCheck className="size-4" />Nessun tipo legacy da classificare.</p> : unclassifiedTypes.map((type) => { const legacyCategories = categoriesFor(type.appliesTo as DocumentOwnerType, false); return <form action={classifyType.bind(null, type)} className="grid gap-3 rounded-lg border p-3 sm:grid-cols-[minmax(0,1fr)_minmax(12rem,1fr)_auto] sm:items-end" key={type.id}><div><strong className="text-sm font-medium">{type.name}</strong><p className="mt-1 text-xs text-muted-foreground">Macroarea bloccata: {areas.find((area) => area.ownerType === type.appliesTo)?.label ?? type.appliesTo}</p></div>{legacyCategories.length ? <Field><FieldLabel htmlFor={`classify-${type.id}`}>Categoria confermata</FieldLabel><select className="h-9 w-full rounded-md border bg-background px-3 text-sm" disabled={pending} id={`classify-${type.id}`} name="categoryKey">{legacyCategories.map((category) => <option key={category.key} value={category.key}>{category.label}</option>)}</select></Field> : <p className="text-xs text-muted-foreground">Il tipo legacy non appartiene a una macroarea canonica: richiede una decisione separata.</p>}{canManage && legacyCategories.length ? <Button disabled={pending} type="submit" variant="outline">Classifica tipo</Button> : <Badge variant="warning"><IconLock />Flusso legacy separato</Badge>}</form>; })}</CardContent></Card><Card size="sm"><CardHeader><CardTitle><h3>Documenti senza tipo classificato</h3></CardTitle><CardDescription>{unclassifiedDocuments.length} documenti restano recuperabili e non sono disponibili nel nuovo flusso di creazione.</CardDescription></CardHeader><CardContent className="grid gap-2">{!unclassifiedDocuments.length ? <p className="flex items-center gap-2 text-sm text-muted-foreground"><IconCircleCheck className="size-4" />Nessun documento legacy da classificare.</p> : unclassifiedDocuments.map((document) => <article className="flex flex-col gap-3 rounded-lg border p-3 sm:flex-row sm:items-center sm:justify-between" key={document.id}><div><strong className="text-sm font-medium">{document.title}</strong><p className="mt-1 text-xs text-muted-foreground">{document.documentTypeName ?? "Tipo non assegnato"} · {document.categoryLabel}</p></div><Link className={buttonVariants({ variant: "outline", size: "sm" })} href={documentDetailsHref(document)}><IconFileSettings />Apri e classifica</Link></article>)}</CardContent></Card></section>
