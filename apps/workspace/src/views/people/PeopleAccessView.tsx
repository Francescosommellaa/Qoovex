...
export function PeopleAccessView({ overview, canManage, canRevoke, invitableRoles }: { overview: AccessOverview; canManage: boolean; canRevoke: boolean; invitableRoles: Array<Exclude<OrganizationRole, "OWNER">> }) {
  return (
    <WorkspacePage>
      ...
      <Card size="sm"><CardHeader className="border-b"><CardTitle><h2>Utenti attivi</h2></CardTitle><CardDescription>Account che possono accedere all'azienda.</CardDescription><CardAction><Badge variant="outline">{overview.activeUsers.length}</Badge></CardAction></CardHeader><CardContent>{overview.activeUsers.length ? <ul className="divide-y divide-border">{overview.activeUsers.map((member) => <li className="grid gap-3 py-4 first:pt-0 last:pb-0 sm:grid-cols-[minmax(0,1fr)_auto_auto] sm:items-center" key={member.id}><div className="min-w-0"><p className="truncate text-sm font-medium">{personName(member)}</p><p className="truncate text-sm text-muted-foreground">{member.user.email}</p>{member.role === "COLLABORATOR" ? <p className="mt-1 text-xs text-muted-foreground">{member.preset ?? "Personalizzato"} · {member.scopeMode === "FULL" ? "Tutta l'Azienda" : "Risorse assegnate"} · {member.permissionKeys.length} permessi{member.expiresAt ? ` · scade ${new Date(member.expiresAt).toLocaleDateString("it-IT")}` : ""}</p> : null}</div><WorkspaceState label={roleLabels[member.role]} /><div className="flex flex-wrap gap-2">{canManage && member.role === "COLLABORATOR" ? <Link className={buttonVariants({ variant: "outline", size: "sm" })} data-link="plain" href={`/people/access/${member.id}`}>Modifica accesso</Link> : null}{canRevoke && member.role !== "OWNER" ? <AccessRevokeButton memberId={member.id} /> : null}</div></li>)}</ul> : <Empty className="min-h-40"><EmptyHeader><EmptyMedia variant="icon"><IconUsers aria-hidden="true" /></EmptyMedia><EmptyTitle>Nessun utente attivo</EmptyTitle><EmptyDescription>Gli account abilitati appariranno qui.</EmptyDescription></EmptyHeader></Empty>}</CardContent></Card>
