  ...
  return (
    <WorkspacePage>
      ...
      <Card><CardContent className="mx-auto grid max-w-4xl gap-5">
        ...
        {step === 0 ? <div className="grid gap-5"><Field><FieldLabel htmlFor="member-preset">Preset iniziale</FieldLabel><select className="h-9 rounded-md border bg-background px-3 text-sm" id="member-preset" onChange={(event) => setPreset(event.target.value as OrganizationAccessPreset)} value={preset ?? "CUSTOM"}>{presets.map((value) => <option key={value} value={value}>{value}</option>)}</select><FieldDescription>Il preset descrive la configurazione iniziale; l'autorizzazione usa sempre i permessi persistiti.</FieldDescription></Field><Field><FieldLabel>Perimetro</FieldLabel><div className="grid gap-2 sm:grid-cols-2">{(["FULL", "ASSIGNED"] as const).map((value) => <button className={`rounded-lg border p-3 text-left text-sm ${scopeMode === value ? "border-primary/40 bg-primary/5" : ""}`} key={value} onClick={() => setScopeMode(value)} type="button">{value === "FULL" ? "Tutta l'Azienda" : "Solo risorse selezionate"}</button>)}</div></Field>{scopeMode === "ASSIGNED" ? resources.map((group) => <fieldset className="grid gap-2 rounded-xl border p-4" key={group.label}><legend className="px-1 text-sm font-medium">{group.label}</legend>{group.items.length ? <div className="grid gap-2 sm:grid-cols-2">{group.items.map((item) => { const checked = grants.some((grant) => grant.resourceType === item.resourceType && grant.resourceId === item.id); return <label className="flex items-center gap-2 text-sm" key={`${item.resourceType}:${item.id}`}><Checkbox checked={checked} onCheckedChange={(next) => setGrants((current) => next === true ? [...current, { resourceType: item.resourceType, resourceId: item.id }] : current.filter((grant) => grant.resourceType !== item.resourceType || grant.resourceId !== item.id))} />{item.label}</label>; })}</div> : <p className="text-sm text-muted-foreground">Nessuna risorsa disponibile.</p>}</fieldset>) : null}<Field><FieldLabel htmlFor="member-access-expiry">Scadenza accesso opzionale</FieldLabel><Input id="member-access-expiry" onChange={(event) => setExpiresAt(event.target.value)} type="datetime-local" value={expiresAt} /></Field></div> : null}
