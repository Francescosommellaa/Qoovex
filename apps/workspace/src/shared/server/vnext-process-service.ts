...
import { VNEXT_PROCESS_DEFINITIONS, type VNextProcessDefinition } from "./vnext-registry";
import { runSerializableTransaction } from "./serializable-transaction";

const LEASE_MS = 60_000;
const MAX_RUN_BATCH = 10;

export async function enqueueVNextProcess(input: {
  organizationId: string;
  jobSiteId: string;
  definitionKey: VNextProcessDefinition;
  businessKey: string;
  payload: Record<string, unknown>;
}) {
  const activeKey = `${input.jobSiteId}:${input.definitionKey}:${input.businessKey}`;
  const definition = VNEXT_PROCESS_DEFINITIONS[input.definitionKey];
...
async function executeProcessEffect(process: NonNullable<Awaited<ReturnType<typeof claimOne>>>) {
  ...
  const { generateJobSiteExport } = await import("./vnext-export-service");
...
export async function runVNextProcesses(workerId = randomUUID()) {
  let scanned = 0;
  let completed = 0;
  ...
  const { runVNextRetentionCleanup } = await import("./vnext-retention-service");
  const { runVNextNotificationDeliveries } = await import("./vnext-notification-service");
  const { expireChangeProposals } = await import("./vnext-collaboration-service");
  const [retention, deliveries, expiredProposals] = await Promise.all([runVNextRetentionCleanup(), runVNextNotificationDeliveries(), expireChangeProposals()]);
