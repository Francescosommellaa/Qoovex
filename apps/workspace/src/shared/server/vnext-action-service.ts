...
import { canonicalize, effectPayloadSchemas, fingerprintPayload, vnextActionInputSchema, type VNextActionInput } from "./vnext-contracts";
...
async function requireAuthority(actor: VNextActor, capability: "COMMERCIAL_NEGOTIATE" | "COMMERCIAL_ACCEPT" | "PAYMENT_REQUEST" | "PAYMENT_CONFIRM_RECEIPT") {
...
async function applyProposalEffects(tx: Prisma.TransactionClient, actor: VNextActor, effects: Array<{ id: string; type: keyof typeof effectPayloadSchemas; payload: unknown }>) {
...
async function executeActionBody(tx: Prisma.TransactionClient, actor: VNextActor, input: VNextActionInput, internal: boolean) {
...
export async function executeVNextAction(input: {
  actor: VNextActor;
  ...
  const action = vnextActionInputSchema.parse(input.action);
  ...
  if (action.action === "JOB_SITE_CLOSE@1") {
    ...
    await Promise.all(exportIds.map((exportId) => enqueueVNextProcess({ organizationId: input.actor.organizationId, jobSiteId: input.actor.jobSiteId, definitionKey: "JOB_SITE_EXPORT@1", businessKey: exportId, payload: { exportId } })));
  }
