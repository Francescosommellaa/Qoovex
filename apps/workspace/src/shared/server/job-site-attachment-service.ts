...
import { resolveClientJobSiteActor, resolveOrganizationJobSiteActor, requireEconomicAuthority, revalidateActor, type JobSiteActor } from "./job-site-authorization-service";
...
export type JobSiteAttachmentCategory = keyof typeof MIME_BY_CATEGORY;
export const jobSiteAttachmentCategorySchema = z.enum(Object.keys(MIME_BY_CATEGORY) as [JobSiteAttachmentCategory, ...JobSiteAttachmentCategory[]]);
...
async function requireRelatedRecord(tx: Prisma.TransactionClient, input: { actor: JobSiteActor; category: JobSiteAttachmentCategory; relatedId: string | null }) {
  if (!input.relatedId) {
...
export async function uploadJobSiteAttachment(input: { actor: JobSiteActor; file: File; category: JobSiteAttachmentCategory; audience: "INTERNAL" | "SHARED"; expectedRevision: number; idempotencyKey: string; relatedId?: string | null }) {
...
export async function downloadJobSiteAttachment(input: { actor: JobSiteActor; attachmentId: string }) {
  await revalidateActor(input.actor);
