...
vi.mock("@shared/server/qoovex-operator-access", () => ({ requireQoovexOperatorById: mocks.requireById }));
vi.mock("@qoovex/db", () => ({
  db: {
...
beforeEach(() => {
  ...
  mocks.requireById.mockReset().mockResolvedValue({ id: "admin-1", email: "admin@qoovex.com", isDev: false, platformRole: "SUPER_ADMIN" });
...
describe("platform user management", () => {
  ...
  it("refuses to modify the actor or another SUPER_ADMIN", async () => {
    mocks.userFindUnique.mockResolvedValue({ id: "admin-1", email: "admin@qoovex.com", platformRole: "SUPER_ADMIN", suspendedAt: null });
    await expect(suspendPlatformUser("admin-1", "Motivo operativo valido")).rejects.toMatchObject({ status: 409 });
    mocks.userFindUnique.mockResolvedValue({ id: "admin-2", email: "two@qoovex.com", platformRole: "SUPER_ADMIN", suspendedAt: null });
