...
if (process.env.CI && !environment.isLocalCiDatabase) {
  throw new Error("CI must run current JobSite creation tests against the local qoovex_ci PostgreSQL database.");
}
...
import { resolveOrganizationJobSiteActor } from "./job-site-authorization-service";
import { createJobSite, invitePrimaryClientIdempotent } from "./job-site-lifecycle-service";
...
describeOnLocalCi("current JobSite creator activation on PostgreSQL", () => {
  it("creates one active organization participant and lets the creator invite the client immediately", async () => {
    ...
    const created = await createJobSite(owner.organization.id, idempotencyKey, {
    ...
    const replay = await createJobSite(owner.organization.id, idempotencyKey, {
      name: "Cantiere creator attivo",
      address: "Via del test 1",
    });
  ...
  it("rolls back the JobSite and participant when the creation transaction fails", async () => {
    ...
    await expect(createJobSite(owner.organization.id, idempotencyKey, {
