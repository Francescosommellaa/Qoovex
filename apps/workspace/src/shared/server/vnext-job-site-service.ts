...
import type { VNextActor } from "./vnext-authorization-service";
...
export async function createVNextJobSite(organizationId: string, idempotencyKey: string, rawInput: unknown) {
  const context = await requireOrganizationContext(organizationId);
...
export async function invitePrimaryClientIdempotent(input: { actor: VNextActor; idempotencyKey: string; rawInput: unknown }) {
  ...
  if (!execution.replayed) {
    await enqueueVNextProcess({ organizationId: input.actor.organizationId, jobSiteId: input.actor.jobSiteId, definitionKey: "CLIENT_INVITATION@1", businessKey: execution.result.invitationId, payload: { invitationId: execution.result.invitationId } });
...
export async function publishInitialAgreementIdempotent(input: { actor: VNextActor; idempotencyKey: string; rawInput: unknown }) {
...
export async function revokePrimaryClientInvitation(input: { actor: VNextActor; idempotencyKey: string; rawInput: unknown }) {
...
export async function transitionClientParticipation(input: { actor: VNextActor; idempotencyKey: string; rawInput: unknown }) {
