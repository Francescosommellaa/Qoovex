...
const jobSiteService = source("src/shared/server/vnext-job-site-service.ts");
const accessContext = source("src/shared/server/access-context-service.ts");
const authorization = source("src/shared/server/vnext-authorization-service.ts");
const actionService = source("src/shared/server/vnext-action-service.ts");
const collaboration = source("src/shared/server/vnext-collaboration-service.ts");
const idempotency = source("src/shared/server/vnext-idempotency-service.ts");
...
describe("first vertical slice - verified green guardrails", () => {
  it("derives organization, actor and permission server-side for creation and invitation", () => {
    const create = between(jobSiteService, "export async function createVNextJobSite", "export async function getOrganizationJobSiteDetail");
    expect(createRoute).toContain("createVNextJobSite((await params).organizationId");
