...
export async function buildDataExportForOrganization(organizationId: string, generatedAt = new Date()): Promise<DataExportResponse> {
  ...
  return { generatedAt: generatedAt.toISOString(), organization, members, workers, jobSites, workerUserLinks, jobSiteParticipants, jobSiteWorkerAssignments, documents, documentVersions, documentJobSiteLinks, evidence, evidenceRevisions, notifications, auditEvents, invitations, dataControlJobs, supportSessions, supportEvents, jobSite } as unknown as DataExportResponse;
}
