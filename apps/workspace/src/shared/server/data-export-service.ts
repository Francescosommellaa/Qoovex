...
export async function buildDataExportForOrganization(organizationId: string, generatedAt = new Date()): Promise<DataExportResponse> {
  ...
  const [workers, jobSites, workerUserLinks, jobSiteParticipants, jobSiteWorkerAssignments, notifications, auditEvents, invitations, dataControlJobs, supportSessions, supportEvents] = await Promise.all([
  ...
  const jobSite = { clientInvitations, authorityGrants, agreements, steps, timeline, attachments, requests, proposals, paymentProfiles, paymentRequests, disputes, closures, postClosureRequests, reopeningProposals, exports, legalHolds, actionReceipts, processes, notificationPreferences, notificationDeliveries };
  return { generatedAt: generatedAt.toISOString(), organization, members, workers, jobSites, workerUserLinks, jobSiteParticipants, jobSiteWorkerAssignments, notifications, auditEvents, invitations, dataControlJobs, supportSessions, supportEvents, jobSite } as unknown as DataExportResponse;
}
