...
import { appendTimelineSchema, closureSnapshotSchema, createProposalSchema, createStepSchema, effectPayloadSchemas, fingerprintPayload } from "./vnext-contracts";
import { executeIdempotentJobSiteMutation } from "./vnext-idempotency-service";
...
export async function listTimeline(input: { actor: VNextActor; cursor: string | null }) {
...
export async function changeJobSiteResponsible(input: { actor: VNextActor; idempotencyKey: string; body: unknown }) {
...
import { revalidateActor, requireEconomicAuthority, type VNextActor } from "./vnext-authorization-service";
...
export async function appendTimeline(input: { actor: VNextActor; idempotencyKey: string; body: unknown }) {
...
export async function createStep(input: { actor: VNextActor; idempotencyKey: string; body: unknown; expectedRevision: number }) {
  if (input.actor.side !== "ORGANIZATION_MEMBER") throw new AccessError("Azione riservata all'Azienda.", 403);
...
export async function createStructuredRequest(input: { actor: VNextActor; idempotencyKey: string; body: unknown }) {
...
export async function transitionStructuredRequest(input: { actor: VNextActor; requestId: string; idempotencyKey: string; body: unknown }) {
...
export async function createChangeProposal(input: { actor: VNextActor; idempotencyKey: string; body: unknown }) {
  ...
  await enqueueVNextProcess({ organizationId: input.actor.organizationId, jobSiteId: input.actor.jobSiteId, definitionKey: "CHANGE_NEGOTIATION@1", businessKey: String(execution.result.proposalId), payload: { proposalId: execution.result.proposalId } });
  return execution;
}
...
export async function counterChangeProposal(input: { actor: VNextActor; proposalId: string; idempotencyKey: string; body: unknown }) {
...
export async function withdrawChangeProposal(input: { actor: VNextActor; proposalId: string; idempotencyKey: string; body: unknown }) {
...
export async function createDispute(input: { actor: VNextActor; idempotencyKey: string; body: unknown }) {
...
export async function transitionDispute(input: { actor: VNextActor; disputeId: string; idempotencyKey: string; body: unknown }) {
...
export async function proposeClosure(input: { actor: VNextActor; idempotencyKey: string; expectedRevision: number }) {
  if (input.actor.side !== "ORGANIZATION_MEMBER") throw new AccessError("Azione riservata all'Azienda.", 403);
  ...
  await enqueueVNextProcess({ organizationId: input.actor.organizationId, jobSiteId: input.actor.jobSiteId, definitionKey: "JOB_SITE_CLOSURE@1", businessKey: String(execution.result.closureId), payload: { closureId: execution.result.closureId } });
  return execution;
}
...
export async function createPostClosureRequest(input: { actor: VNextActor; idempotencyKey: string; body: unknown }) {
  ...
  await enqueueVNextProcess({ organizationId: input.actor.organizationId, jobSiteId: input.actor.jobSiteId, definitionKey: "POST_CLOSURE_REQUEST@1", businessKey: String(execution.result.postClosureRequestId), payload: { postClosureRequestId: execution.result.postClosureRequestId } });
  return execution;
}
...
export async function transitionPostClosureRequest(input: { actor: VNextActor; postClosureRequestId: string; idempotencyKey: string; body: unknown }) {
...
export async function proposeReopening(input: { actor: VNextActor; idempotencyKey: string; body: unknown }) {
  ...
  await enqueueVNextProcess({ organizationId: input.actor.organizationId, jobSiteId: input.actor.jobSiteId, definitionKey: "JOB_SITE_REOPENING@1", businessKey: String(execution.result.reopeningProposalId), payload: { reopeningProposalId: execution.result.reopeningProposalId } });
  return execution;
}
...
export async function grantEconomicAuthority(input: { actor: VNextActor; idempotencyKey: string; rawInput: unknown }) {
...
export async function revokeEconomicAuthority(input: { actor: VNextActor; idempotencyKey: string; rawInput: unknown }) {
  if (input.actor.role !== "OWNER") throw new AccessError("Risorsa non disponibile.", 404);
...
export async function searchSharedClientContent(actor: VNextActor, query: string) {
...
export async function searchOrganizationContent(actor: VNextActor, query: string) {
