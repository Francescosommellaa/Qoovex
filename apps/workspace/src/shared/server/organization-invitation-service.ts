...
import { getContextOrganizationId, getViewerContext, requireIdentity } from "@shared/server/access-context-service";
...
const INVITABLE_ROLES = new Set<OrganizationRole>(["ADMIN", "SAFETY_CONSULTANT", "SITE_MANAGER", "WORKER", "VIEWER"]);
...
export async function createInvitation(input: { email: string; role: OrganizationRole }) {
  ...
  const context = await getViewerContext();
  const actorRole = context.support ? "OWNER" : context.membership?.role;
  ...
  if (existingUser) {
    const membership = await db.organizationMembership.findFirst({ where: { userId: existingUser.id, revokedAt: null }, select: { id: true } });
    if (membership) throw new AccessError("Questo utente appartiene gia a una azienda.", 409);
  }
...
export async function acceptInvitation(rawToken: string) {
  ...
  const active = await db.organizationMembership.findFirst({ where: { userId: user.id, revokedAt: null }, select: { id: true } });
  if (active) throw new AccessError("Appartieni gia a una azienda.", 409);
