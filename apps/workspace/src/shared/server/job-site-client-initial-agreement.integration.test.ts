...
if (process.env.CI && !environment.isLocalCiDatabase) {
  throw new Error("CI must run current client agreement tests against the local qoovex_ci PostgreSQL database.");
}
...
import { appendTimeline, listTimeline } from "./job-site-collaboration-service";
import { downloadJobSiteAttachment, resolveClientAttachmentActor } from "./job-site-attachment-service";
import { executeJobSiteAction } from "./job-site-action-service";
import {
  resolveClientInitialAgreementActor,
  resolveClientJobSiteActor,
  resolveOrganizationJobSiteActor,
  revalidateActor,
} from "./job-site-authorization-service";
import {
...
} from "./job-site-lifecycle-service";
...
async function createAndInvite(owner: OwnerFixture, client: ClientFixture, name: string) {
  ...
  const created = await createJobSite(owner.organization.id, `create-${crypto.randomUUID()}`, { name });
...
afterAll(async () => {
  ...
  const [organizationCount, ownerCount, clientCount] = await Promise.all([
    ...
    db.user.count({ where: { email: { endsWith: "@job-site-client-owner-test.invalid" } } }),
    db.user.count({ where: { email: { endsWith: "@job-site-client-lifecycle-test.invalid" } } }),
  ]);
  expect({ organizationCount, ownerCount, clientCount }).toEqual({ organizationCount: 0, ownerCount: 0, clientCount: 0 });
});

describeOnLocalCi("current pending client initial agreement lifecycle on PostgreSQL", () => {
  it("keeps acceptance pending, limits pre-activation access and activates only the exact accepted agreement", async () => {
    ...
    await expect(executeJobSiteAction({
      actor: pendingActor,
    ...
    const confirmation = await executeJobSiteAction({
      actor: pendingActor,
  ...
  it("deduplicates concurrent acceptance and confirmation attempts", async () => {
    ...
    const confirmations = await Promise.allSettled([
      executeJobSiteAction({ actor: pendingActor, idempotencyKey: confirmationKey, action }),
      executeJobSiteAction({ actor: pendingActor, idempotencyKey: confirmationKey, action }),
    ]);
    ...
    await expect(executeJobSiteAction({ actor: activeActor, idempotencyKey: confirmationKey, action })).resolves.toMatchObject({ replayed: true });
    const currentRevision = await db.jobSite.findUniqueOrThrow({ where: { id: invitation.jobSiteId }, select: { revision: true } });
    await expect(executeJobSiteAction({ actor: activeActor, idempotencyKey: `repeat-${crypto.randomUUID()}`, action: { ...action, expectedRevision: currentRevision.revision } })).rejects.toMatchObject({ status: 409 });
    await expect(db.jobSiteInitialAgreementConsent.count({ where: { participantId: participant.id, versionId: published.agreementVersionId } })).resolves.toBe(1);
  });
