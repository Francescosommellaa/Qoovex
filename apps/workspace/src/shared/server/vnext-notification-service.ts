...
export async function runVNextNotificationDeliveries(now = new Date()) {
...
export async function queueJobSiteNotifications(tx: Prisma.TransactionClient, input: { actor: VNextActor; action: string; idempotencyKey: string; sourceId?: string | null }) {
  ...
  for (const recipient of recipients) {
    ...
    if (inAppFrequency !== "DISABLED") {
      const dedupeKey = inAppFrequency === "DAILY_DIGEST" ? `vnext-digest:${input.actor.organizationId}:${category.type}:${day}:${recipient.userId}` : `vnext:${input.action}:${input.idempotencyKey}:${recipient.userId}`;
    ...
    if (emailFrequency !== "DISABLED") {
      ...
      const dedupeKey = digest ? `vnext-email-digest:${input.actor.organizationId}:${category.type}:${day}:${recipient.userId}` : `vnext-email:${input.action}:${input.idempotencyKey}:${recipient.userId}`;
