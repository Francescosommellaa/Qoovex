...
import { canonicalize, fingerprintPayload, initialAgreementPayloadSchema, proposalPayloadSchema, vnextActionInputSchema } from "./vnext-contracts";

describe("vNext versioned public contracts", () => {
  ...
  it("serializes money from strings and forbids generic actions", () => {
    const payment = vnextActionInputSchema.parse({ action: "PAYMENT_REQUEST_CREATE@1", expectedRevision: 1, paymentProfileId: "profile", amountMinor: "12345", reason: "Acconto", dueAt: null, stepIds: [], proposalIds: [] });
    ...
    expect(vnextActionInputSchema.safeParse({ action: "GENERIC_UPDATE", expectedRevision: 1 }).success).toBe(false);
  });
