...
export default async function NotificationPreferencesPage() {
  ...
  return <WorkspacePage><WorkspacePageHeader title="Preferenze notifiche" description="Le preferenze sono personali e separate per contesto." action={<Link className={buttonVariants({ variant: "outline" })} href="/contexts">Torna ai contesti</Link>} /><div className="grid gap-4 lg:grid-cols-[22rem_1fr]"><WorkspacePanel><NotificationPreferencesForm contexts={value.contexts} /></WorkspacePanel><WorkspacePanel title="Preferenze correnti">{value.preferences.length ? <ul className="divide-y">{value.preferences.map((preference) => <li className="py-3 text-sm" key={preference.id}>{preference.type} · {preference.channel} · {preference.frequency}</li>)}</ul> : <p className="text-sm text-muted-foreground">Le notifiche usano i valori predefiniti finché non salvi una preferenza.</p>}</WorkspacePanel></div></WorkspacePage>;
}
