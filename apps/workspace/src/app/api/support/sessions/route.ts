...
export async function POST(request: Request) {
  try {
    const [user, body] = await Promise.all([requireIdentity(), request.json() as Promise<{ organizationCode?: string; reason?: string }>]);
    return Response.json(await openSupportSession(user.id, { organizationCode: body.organizationCode, reason: body.reason ?? "" }), { status: 201 });
  } catch (error) { return asAccessResponse(error); }
}
