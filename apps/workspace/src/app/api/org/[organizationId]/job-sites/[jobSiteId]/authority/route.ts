import { grantEconomicAuthority, revokeEconomicAuthority } from "@shared/server/vnext-collaboration-service";
...
export async function DELETE(request: Request, { params }: { params: Promise<{ organizationId: string; jobSiteId: string }> }) {
  try { const value = await params; const actor = await resolveOrganizationJobSiteActor({ ...value, permission: "jobSite:commercial:grant" }); return Response.json(await revokeEconomicAuthority({ actor, idempotencyKey: requireIdempotencyKey(request), rawInput: await request.json() })); } catch (error) { return asVNextApiError(error); }
}
