  ...
  try {
    ...
    return (
      <WorkspacePage>
        ...
        <WorkspacePanel>
          <form className={styles.formRow} action="/qoovex-admin/organizations">
            <div className={styles.field}><label htmlFor="organization-search">Nome, codice o email owner</label><input defaultValue={search.q} id="organization-search" name="q" /></div>
            <button className={styles.button} type="submit">Cerca</button>
          </form>
        </WorkspacePanel>
        <WorkspacePanel title="Risultati">
          {data.organizations.length === 0 ? <WorkspaceEmptyState title="Nessuna azienda" description="Modifica i criteri di ricerca." /> : (
            <div className={styles.recordList}>{data.organizations.map((organization) => (
              <article className={styles.record} key={organization.id}>
                ...
                <p className="qv-text-muted">{organization._count.memberships} membri · {organization._count.workers} lavoratori · {organization._count.jobSites} cantieri · {organization._count.documents} documenti</p>
                <p className="qv-text-muted">Owner: {organization.memberships.map((membership) => membership.user.email).join(", ") || "non disponibile"}</p>
