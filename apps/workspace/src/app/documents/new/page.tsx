...
export default async function NewDocumentPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  try {
    ...
    return <AdminCreationPage title="Aggiungi documento" description="Scegli destinazione, categoria e tipo prima di confermare il salvataggio." backHref="/documents" backLabel="Annulla" panelTitle="Documento" panelDescription="Il percorso guidato impedisce di salvare un documento senza classificazione."><DocumentCreateFlow canManageTypes={capabilities.canManageDocumentSettings} documentTypes={serializeForClient<WorkspaceDocumentTypeRecord[]>(documentTypes)} workers={serializeForClient<WorkspaceWorkerRecord[]>(workers)} jobSites={serializeForClient<WorkspaceJobSiteRecord[]>(jobSites)} initialContext={flow.context} origin={flow.origin} /></AdminCreationPage>;
