...
La shell mantiene piccola la navigazione quotidiana e raccoglie le anagrafiche nel gruppo `Persone`: `Lavoratori` contiene i profili operativi, mentre `Utenti e inviti` contiene account, ruolo assegnato e inviti. Le assegnazioni ai cantieri restano contestuali e non vengono duplicate nella sidebar o nei Preferiti.

`Preferiti` contiene esclusivamente viste operative autorizzate, con due default espliciti per ruolo e un massimo di quattro scelte. La configurazione usa `localStorage` con chiave versionata e role-scoped, non Prisma; la shell non esegue query o conteggi per Preferiti e Azioni rapide. `Azioni rapide` resta il blocco separato per le creazioni. Le notifiche, incluso il badge non letto, sono accessibili soltanto dalla topbar e non sono candidate o destinazioni della sidebar.
...
Il workspace admin core espone pagine prodotto per `/documents`, `/deadlines`, `/workers` e `/job-sites`, con dettaglio documento/lavoratore/cantiere. Cantieri usa `/job-sites` come panoramica decisionale, `/job-sites/all` come elenco filtrato e paginato e `/job-sites/archive` come consultazione; la creazione e un Dialog guidato. Il dettaglio conserva slug leggibile, URL legacy e sette sezioni server-rendered selezionate con `?section=`. L'estensione admin aggiunge `/checklists`, `/evidence` e `/document-packages`, inclusa gestione voci checklist, prove e share link. La pagina `/notifications` mostra promemoria interni derivati da scadenze registrate, stati documentali, pacchetti e share link. La rifinitura UX mantiene nav completa, layout mobile-first, empty state, error/loading states e controlli statici su copy e dati sensibili. Le mutation usano endpoint API gia protetti e la UI non invia `organizationId` dal client.
...
Le API Worker e JobSite gestiscono solo metadati operativi minimi, filtrati per `Organization`. JobSite include una fase operativa esplicita e puo creare contestualmente le relazioni di responsabili e lavoratori. Non gestiscono presenze, geolocalizzazione, dati sanitari o responsabilita duplicate sul record.
