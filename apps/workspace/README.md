...
La shell espone al massimo quattro destinazioni quotidiane: `Da fare`, `Cantieri`, `Lavoratori` e `Documenti` per OWNER, ADMIN e SAFETY_CONSULTANT; SITE_MANAGER e WORKER non vedono `Lavoratori` nella navigazione primaria. Notifiche con conteggio, menu `Aggiungi` e menu azienda/account sono separati. Le route globali per scadenze, checklist, prove, condivisioni e accessi restano raggiungibili come superfici secondarie o contestuali.
...
La route `/access` permette a `OWNER` e `ADMIN` di creare collegamenti operativi utente-lavoratore e assegnazioni cantiere; `SAFETY_CONSULTANT` puo consultarli in sola lettura in base al permesso `assignments:read`. `SITE_MANAGER` e `WORKER` leggono solo risorse assegnate tramite filtri server-side; la lettura esterna resta confinata agli share link.
