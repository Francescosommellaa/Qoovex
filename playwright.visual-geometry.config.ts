...
const projects = [
  ["sirio", "light", `http://127.0.0.1:${ports.sirio}`],
  ["sirio", "dark", `http://127.0.0.1:${ports.sirio}`],
  ["web", "light", `http://127.0.0.1:${ports.web}`],
  ["web", "dark", `http://127.0.0.1:${ports.web}`],
  ["workspace", "light", `http://127.0.0.1:${ports.workspace}`],
  ["workspace", "dark", `http://127.0.0.1:${ports.workspace}`],
] as const;
...
export default defineConfig({
  ...
  workers: 1,
  retries: 0,
  // Only the attested update runner may override this, including missing fixtures.
  ...
  webServer: [
    {
      command: `pnpm --filter @qoovex/web exec next start --port ${ports.web}`,
      url: `http://127.0.0.1:${ports.web}`,
      reuseExistingServer: false,
      timeout: 60_000,
    },
    {
      command: `pnpm --filter @qoovex/workspace exec next start --port ${ports.workspace}`,
      env: {
        ...process.env,
        ...
        AUTH_URL: `http://127.0.0.1:${ports.workspace}`,
        QOOVEX_VISUAL_GEOMETRY: "1",
      },
      url: `http://127.0.0.1:${ports.workspace}/sign-in`,
      reuseExistingServer: false,
      timeout: 60_000,
    },
    {
      command: `pnpm --filter @qoovex/sirio exec next start --port ${ports.sirio}`,
      url: `http://127.0.0.1:${ports.sirio}/components/button`,
      reuseExistingServer: false,
      timeout: 60_000,
    },
  ],
