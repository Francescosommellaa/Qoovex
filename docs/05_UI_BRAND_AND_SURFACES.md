...
Nella route Workspace `/workers`, `Aggiungi lavoratore` e `Apri profilo` sono trigger del `Dialog` canonico e non cambiano pagina. Il dettaglio riusa il record gia caricato dalla lista e quindi non aggiunge una lettura all'apertura; la gestione completa resta una destinazione esplicita soltanto per OWNER e ADMIN. La pagina completa usa la stessa gerarchia visuale di Gestisci documento per riepilogo, azioni reali, relazioni e progressive disclosure della gestione. L'URL `/workers/[nome-normalizzato]--[id]` e il breadcrumb derivano dal record gia caricato; gli URL legacy restano validi. Il form di creazione separa `Mansione` da `Accesso a Qoovex`: il primo e testo operativo libero, il secondo mostra soltanto i ruoli realmente invitabili dall'attore. Se il profilo viene creato ma l'invito fallisce, il Dialog consente di ritentare senza creare un duplicato.
...
Le route `/settings/people` e `/access` hanno responsabilita diverse. `Utenti e inviti` mantiene separati account abilitati e inviti in attesa, mostra il ruolo assegnato e non rimanda a una seconda anagrafica. `Assegnazioni cantieri` espone responsabili e lavoratori collegati ai cantieri; la relazione account-profilo WORKER resta una disclosure avanzata che spiega esplicitamente di non assegnare o modificare il ruolo. I form compongono `Field`, `Select` e `Button` condivisi, mentre la modalita di consultazione rimuove le affordance di modifica senza simulare controlli disabilitati. Nessuna card non interattiva introduce hover.
