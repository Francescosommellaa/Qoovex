...
- `Button` e `buttonVariants` usano il cursore pointer sui controlli azionabili; una superficie non interattiva non introduce hover, sollevamento o altri segnali di azione.
...
La sidebar segue una gerarchia task-first: `Ricerca`, `Da fare` e l'eventuale `Analisi` precedono il gruppo `Workspace`; Documenti e Persone sono gruppi espandibili, mentre Calendario e Cantieri restano destinazioni dirette. Documenti contiene Panoramica, Azienda, Lavoratori, Cantieri e, soltanto per i ruoli autorizzati, Pacchetti. Tutte le voci usano esclusivamente route e visibilita derivate dalla policy server-side. `Persone` e espandibile ma contiene soltanto destinazioni anagrafiche: `Lavoratori` per i profili operativi e, per OWNER/ADMIN, `Utenti e inviti` per account, ruolo assegnato e inviti. SITE_MANAGER vede soltanto Lavoratori; WORKER non riceve il gruppo. Le assegnazioni ai cantieri non sono duplicate nella sidebar o nei Preferiti e restano raggiungibili dai contesti operativi. Ricerca e Analisi sono marcate `Presto` e non simulano funzioni non implementate.

`Preferiti` raccoglie code e viste operative personali, non mutazioni. Ogni ruolo riceve due default espliciti e un catalogo permission-aware; l'utente mantiene fino a quattro scelte tramite `Personalizza Preferiti`. La chiave `localStorage` v2 e stabile e role-scoped, la migrazione precedente e idempotente e ogni href viene risanitizzato contro i candidati trusted. Titolo, label, tooltip e menu checkbox restano accessibili su desktop, rail collassata e drawer mobile. Non esistono contatori live, query Prisma, persistenza cloud o voce Notifiche nella sezione.

La creazione globale usa l'action tray `Azioni rapide`, ancorato nel footer subito sopra `Azienda e account`: la posizione lo separa dal flusso Workspace/Preferiti e una superficie `sidebar-accent` con ring tokenizzato comunica un comportamento operativo distinto dalla navigazione. Tutte le creazioni consentite dalla policy restano one-click. Su desktop aperto sono una sola riga iconica, nella rail diventano una colonna raccolta e nel drawer touch una griglia a due colonne con etichette. Documento e Cantiere usano le icone Tabler composte `FilePlus` e `BuildingPlus`; lavoratore, prova, checklist e condivisione mantengono simboli specifici. Tooltip, nome accessibile e focus spiegano ogni destinazione, senza un generico `+` ambiguo. Ogni destinazione chiude il drawer mobile dopo il cambio route. La campanella, il pannello e il badge delle notifiche restano esclusivamente nella topbar e non sono duplicati nella sidebar.

Sidebar e topbar occupano il viewport e non scorrono con la pagina: soltanto il contenuto centrale usa overflow verticale. Il toggle desktop sposta solo la propria icona insieme alla transizione canonica della sidebar. Il breadcrumb desktop e anche una cronologia di navigazione session-scoped: conserva al massimo tre route distinte, mantiene la pagina corrente come ultimo elemento e rende cliccabili le precedenti. Le pagine di risorsa possono sostituire l'etichetta generica con il titolo autorizzato gia disponibile nel render; gli URL leggibili e legacy di documenti e lavoratori vengono considerati la stessa destinazione. Su mobile il logo viene rimosso dalla topbar e il breadcrumb fisso `Da fare` resta il ritorno rapido alla dashboard.
...
`Assegna` non porta direttamente alla pagina completa: per risorse lavoratore o cantiere senza responsabile apre un `Dialog` contestuale con responsabile attuale, scelta della persona, loading, vuoto, errore e conferma. Il salvataggio aggiorna la dashboard; `Gestisci assegnazioni` resta una destinazione secondaria esplicita verso la superficie contestuale.
...
La composizione usa viste mese, settimana, giorno e agenda, slot orari, selezione touch, drag e resize, filtri per scadenze, task, priorita e persona, Dialog accessibili per CRUD e integrazione iCalendar. Lo switcher app-local riprende il linguaggio della navigazione marketing con un indicatore trasformato in 260 ms; il cambio vista aggiunge solo un breve feedback di opacita e posizione. `prefers-reduced-motion` disattiva entrambi. `Oggi` aggiorna il periodo e centra nel viewport la cella corrente. La barra `Nessun risultato` compare soltanto quando una categoria o persona selezionata non produce elementi, offre `Azzera filtri` e non si sovrappone alla griglia; nello stato predefinito il calendario resta privo di avvisi persistenti.
...
Nella route Workspace `/documents`, i filtri di stato ripetono il segmented control del Calendario: gruppo su superficie `muted`, selezione primaria, alternative ghost e scorrimento orizzontale controllato su viewport stretti. `Aggiungi documento` e un trigger del Dialog canonico: compone il form esistente, gestisce loading ed errore/retry dei tipi documento e non cambia URL. `Archivio documenti` e un'azione separata e riservata a OWNER e ADMIN che apre `/documents/archive`. Le due viste usano la stessa struttura `Vista documenti`: la lista principale filtra per stato, mentre l'archivio contiene gia soltanto record `ARCHIVED` e filtra per contesto (Azienda, Lavoratori, Cantieri). La pagina archivio comunica il rischio con il bordo rosso delle card, evitando fondo colorato, badge laterali, avvisi testuali ridondanti e hover su card non cliccabili. Le card archiviate espongono lo stesso Dialog dettaglio in modalita sola lettura, senza descrizione ridondante visibile, freccia di navigazione, upload, pagina di gestione o richiesta file aggiuntiva, oltre a ripristino primario ed eliminazione distruttiva. Il Dialog di eliminazione usa X e backdrop come dismiss standard, mostra il titolo esatto in una riga informativa selezionabile con copia esplicita e richiede di incollarlo nel campo prima di abilitare la CTA irreversibile; bordo distruttivo, footer neutro e CTA rossa attiva mantengono chiaro il contesto di rischio.

Nella route Workspace `/workers`, `Aggiungi lavoratore` apre il Dialog canonico con quattro passaggi: dati operativi, accesso, cantieri e riepilogo. Il ruolo del percorso accesso e bloccato su WORKER; l'utente puo scegliere esplicitamente solo profilo oppure invito e puo assegnare cantieri o rimandare. La rubrica e server-side, espone stato documentale, accesso derivato, cantieri e prossima azione; il dettaglio parte dal riepilogo e organizza documenti, cantieri, scadenze, prove e accesso. In caso di errore successivo alla creazione, il Dialog mostra cosa e stato salvato e ritenta soltanto i passaggi mancanti.

Nella route Workspace `/job-sites`, `Aggiungi cantiere` e `Apri cantiere` riusano lo stesso pattern Dialog. L'anteprima mostra stato, committente, indirizzo, periodo e note usando il record gia caricato, senza fetch all'apertura; la CTA primaria apre la superficie operativa completa e cambia etichetta in `Gestisci cantiere` quando l'attore puo modificare. Nella pagina completa il `Responsabile cantiere` compare sia nel riepilogo sia in una Card primaria dedicata, prima delle azioni operative; assegnazione e stato corrente restano contestuali e permission-scoped. I lavoratori sono una relazione separata e le eventuali etichette libere vengono dichiarate come `Mansione`, senza trasformarle in ruoli di accesso. Documento, prova, scadenza, checklist e condivisione si aprono in Dialog contestuali con il cantiere preselezionato; condividono la stessa anatomia con riepilogo del contesto, primitive form canoniche, feedback `Alert`/`Spinner` e azioni nel `DialogFooter`, senza ripetere select disabilitate. Il Dialog documento carica i tipi configurati soltanto alla prima apertura. Modifica e archiviazione sono raccolte in progressive disclosure e restano soggette ai permessi esistenti. L'URL `/job-sites/[nome-normalizzato]--[id]` e il breadcrumb derivano dal cantiere, mentre gli URL legacy basati sul solo ID restano validi.

Ogni superficie che compone `EvidenceForm` eredita lo stesso ingresso immagini. `Foto collegata` e la scelta iniziale; il file picker locale resta disponibile su desktop e mobile, mentre sotto il breakpoint mobile compare anche `Scatta una foto`, collegato alla fotocamera posteriore mediante l'attributo HTML `capture="environment"`. I due ingressi convergono nell'unico payload Evidence esistente e non introducono primitive, endpoint o contratti storage alternativi.

La sidebar contiene la sezione collassabile Persone. OWNER e ADMIN vedono Panoramica, Lavoratori, Accessi e Assegnazioni; SAFETY_CONSULTANT vede Panoramica, Lavoratori e Assegnazioni in sola lettura; SITE_MANAGER vede Lavoratori; WORKER vede `Il mio profilo`. `/people/access` mantiene separati account, membership, ruoli, inviti e configurazioni incomplete. `/people/assignments` espone esclusivamente responsabili e lavoratori per cantiere: il collegamento account-profilo WORKER si gestisce nel dettaglio del lavoratore o nasce automaticamente dall'invito. Le route legacy `/settings/people` e `/access` reindirizzano alle nuove superfici.
# Aggiornamento superficie Cantieri 2026-07-23

Questa sezione sostituisce le precedenti descrizioni UI di Cantieri in questo documento. Cantieri e un gruppo collassabile role-aware; la panoramica mostra fasi dichiarate, coda di attenzione e attivita recente. Elenco completo e archivio sono superfici separate. Il wizard in Dialog espone cinque passaggi, warning duplicati con conferma esplicita e scelta del prossimo passo senza side effect automatici.

Il dettaglio conserva lo slug e gli URL legacy, ma presenta `Riepilogo`, `Documenti`, `Persone`, `Attivita e scadenze`, `Prove`, `Condivisioni` e `Impostazioni` come navigazione orizzontale scorrevole e URL-shareable. Ogni sezione riusa Card, Badge, Empty, Alert, Dialog e form canonici; le azioni contestuali mantengono il cantiere bloccato e i record archiviati sono sola lettura.
