...
La history Prisma contiene otto migration canoniche. Le migration forward pubblicate rimuovono fisicamente le tabelle autonome del prodotto precedente, aggiungono `AccountRole`, mantengono `JobSiteAttachment` per i file contestuali e vincolano ogni account a una sola membership Azienda attiva. Blob è privato; upload e download passano dal server, applicano autorizzazione oggetto, audit, pathname generato, checksum SHA-256, MIME reale e limite tecnico 4 MiB.
...
Nessuna cancellazione fisica automatica dei record correnti; nessun cleanup di contenuti soggetti a hold; nessuna chiave reale versionata. Blob non viene azzerato durante i normali rilasci forward: ogni rimozione di oggetti richiede un task separato, target esplicito e prova di non-riferimento.
