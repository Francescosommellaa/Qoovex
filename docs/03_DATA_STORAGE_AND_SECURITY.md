...
La migration incrementale `20260720010000_calendar_events` aggiunge il dominio calendario, enum, relazioni, indici e azioni audit. Ogni deploy resta soggetto al wrapper protetto, al backup verificato e al controllo della cronologia.

La cronologia canonica parte da `20260712010000_single_company_baseline`, gia registrata nel database condiviso, e prosegue solo con migration incrementali. L'ultima migration locale e `20260713020000_rate_limit_privacy_atomicity`: elimina le chiavi rate-limit legacy contenenti PII e aggiunge attribuzione utente e retention; resta soggetta al rollout protetto e non va applicata automaticamente. Le migration applicate sono immutabili: nome, ordine e checksum devono coincidere con `_prisma_migrations`; non usare `migrate resolve` per nascondere divergenze. Ogni evoluzione schema richiede migration dedicata e verifica Prisma. Non introdurre Supabase, Firebase, S3 o provider storage alternativi.
