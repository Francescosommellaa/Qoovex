...
`@qoovex/ui#test` e un guardrail statico cross-repository: verifica la foundation legacy ancora usata da `apps/web` e `apps/workspace` e, separatamente, l'isolamento della sandbox app-local `apps/sirio`. Il task non usa la cache Turbo perche legge file esterni a `packages/ui` e deve osservare ogni modifica dei consumer.
