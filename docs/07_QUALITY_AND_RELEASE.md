...
Per un aggiornamento intenzionale completo usare `pnpm deps:update`: il primo passaggio aggiorna il progetto root e i riferimenti GitHub Actions, il secondo aggiorna ricorsivamente tutti i package del workspace, entrambi ignorando i range correnti. In seguito eseguire `pnpm check` e lasciare che i job remoti `quality-gate` e `workspace-e2e` verifichino la modifica. `pnpm install` resta invece deterministico: installa quanto dichiarato e non trasforma automaticamente una normale installazione in un upgrade major. La versione dello stesso pnpm si aggiorna separatamente con `pnpm self-update`, che aggiorna il campo `packageManager` consumato anche dalla CI.
