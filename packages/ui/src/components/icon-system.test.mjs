...
const iconButtonClient = read("./icon-button/icon-button-client.tsx");
...
test("icon-only pagination and loaders keep semantics on their owner", () => {
  ...
  assert.match(buttonClient, /<Spinner aria-hidden="true" size="xs" variant="hexagon"/);
  assert.doesNotMatch(buttonClient, /IconLoader2/);
  assert.match(iconButtonClient, /<Spinner aria-hidden="true" size="xs" variant="hexagon"/);
  assert.match(spinner, /qv-spinner-hexagon-segment/);
  for (const removed of ["bars", "dots", "orbit"]) assert.doesNotMatch(spinner, new RegExp(`\\b${removed}:`));
